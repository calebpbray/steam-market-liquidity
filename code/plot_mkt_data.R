...
ggplot(NULL, aes(x=r_date, y=log(median_price))) + 
  ...
  geom_smooth(data=subset(df_cs, r_date >= as.Date("2016-03-29") & r_date <= as.Date("2018-03-29")), method='lm') +
  geom_smooth(data=subset(df_dota, r_date >= as.Date("2016-03-29") & r_date <= as.Date("2018-03-29")), method='lm') +
  geom_smooth(data=subset(df_cs, r_date >= as.Date("2018-03-29") & r_date <= as.Date("2020-03-29")), method='lm', color="green") +
  geom_smooth(data=subset(df_dota, r_date >= as.Date("2018-03-29") & r_date <= as.Date("2020-03-29")), method='lm', color="green")
