...
df_all <- read.csv("../data/combined_items_price_history.csv")

df_cs['r_date'] <- as.Date(df_cs$date, format = "%b %d %Y")
df_dota['r_date'] <- as.Date(df_dota$date, format = "%b %d %Y")
df_all['r_date'] <- as.Date(df_all$date, format = "%b %d %Y")
...
didreg1 = lm(log(median_price) ~ post_treat + treated_unit + post_treat*treated_unit, data=df_all)
summary(didreg1)