...
didreg1 <- feols(log(median_price) ~ post_treat + treated_unit + post_treat:treated_unit | item, weights= ~volume_sold, data=df_all)
summary(didreg1)

didreg2 <- feols(log(median_price) ~ post_treat + treated_unit + post_treat*treated_unit*(rel_rarity+rel_age_mos+pre_sd) | item, weights= ~volume_sold, data=df_all)
summary(didreg2)

didreg3 <- feols(log(volume_sold)~ post_treat + treated_unit + post_treat*treated_unit*(rel_rarity+rel_age_mos+pre_sd) | item, data=df_all)
summary(didreg3)